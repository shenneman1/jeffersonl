	...
	public static function request( $action, $query = array(), $data = array() ) {
		...
		if ( false && ! empty( $timeout_start ) ) {
			...
			if ( $time_remaining < 0 ) {
				...
				return new WP_Error( 'ithemes-updater-timed-out-server', sprintf( _n( 'The server could not be contacted. Requests are delayed for %d minute to allow the server to recover.', 'The server could not be contacted. Requests are delayed for %d minutes to allow the server to recover.', $minutes_remaining, 'it-l10n-solid-backups' ), $minutes_remaining ) );
			}
		}
		...
		if ( ! self::is_valid_response( $response ) ) {
			...
			$message = __( 'Invalid server response.', 'it-l10n-solid-backups' );
			if ( ! is_wp_error( $response ) ) {
				$message = wp_remote_retrieve_response_code( $response ) . ': ' . wp_remote_retrieve_response_message( $response );
			}

			// Translators: %s is the error message.
			return new WP_Error( 'http_request_failed', sprintf( __( 'The server was unable to be contacted. (%s)', 'it-l10n-solid-backups' ), $message ), $response );
		}
		...
		if ( ! empty( $body['error'] ) ) {
			...
			return new WP_Error( $body['error']['type'], sprintf( __( 'An error occurred when communicating with the SolidWP update server: %1$s (%2$s)', 'it-l10n-solid-backups' ), $body['error']['message'], $body['error']['code'] ) );
		}
