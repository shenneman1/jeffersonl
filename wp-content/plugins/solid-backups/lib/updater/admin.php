...
class Ithemes_Updater_Admin {
	...
	public function show_activation_message() {
		...
		echo '<div class="updated fade"><p>' . wp_sprintf( __( 'To receive automatic updates for %l, use the <a href="%s">SolidWP Licensing</a> page found in the Settings menu.', 'it-l10n-solid-backups' ), $names, $url ) . '</p></div>';
	...
	public function add_admin_pages() {
		$this->page_ref = add_options_page( __( 'SolidWP Licensing', 'it-l10n-solid-backups' ), __( 'SolidWP Licensing', 'it-l10n-solid-backups' ), 'manage_options', $this->page_name, array( $this, 'settings_index' ) );

		add_action( "load-{$this->page_ref}", array( $this, 'load_settings_page' ) );
	}

	public function add_network_admin_pages() {
		$this->page_ref = add_submenu_page( 'settings.php', __( 'SolidWP Licensing', 'it-l10n-solid-backups' ), __( 'SolidWP Licensing', 'it-l10n-solid-backups' ), 'manage_options', $this->page_name, array( $this, 'settings_index' ) );
	...
	private function set_registration_link() {
		...
		$url = admin_url( 'options-general.php' ) . "?page={$this->page_name}";
		$this->registration_link = sprintf( '<a href="%1$s" title="%2$s">%3$s</a>', $url, __( 'Manage SolidWP product licenses to receive automatic upgrade support', 'it-l10n-solid-backups' ), __( 'License', 'it-l10n-solid-backups' ) );
	}
