...
class Ithemes_Updater_Settings_Page {
	...
	private function save_settings() {
		...
		$this->messages[] = __( 'Settings saved', 'it-l10n-solid-backups' );
	}
	...
	private function license_packages( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership username and password in order to license products.', 'it-l10n-solid-backups' );
		} else if ( empty( $data['username'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership username in order to license products.', 'it-l10n-solid-backups' );
		} else if ( empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership password in order to license products.', 'it-l10n-solid-backups' );
		} else if ( empty( $data['packages'] ) ) {
			$this->errors[] = __( 'You must select at least one product to license. Ensure that you select the products that you wish to license in the listing below.', 'it-l10n-solid-backups' );
		}
		...
		if ( empty( $response['packages'] ) ) {
			$this->errors[] = __( 'An unknown server error occurred. Please try to license your products again at another time.', 'it-l10n-solid-backups' );
			return;
		}
		...
		foreach ( $response['packages'] as $package => $data ) {
			...
			if ( ! empty( $data['key'] ) ) {
				...
				$warn[ $name ] = __( 'Your product subscription has expired', 'it-l10n-solid-backups' );
			} else {
		...
		if ( ! empty( $success ) ) {
			$this->messages[] = wp_sprintf( __( 'Successfully licensed %l.', 'it-l10n-solid-backups' ), $success );
		}

		if ( ! empty( $fail ) ) {
			foreach ( $fail as $name => $reason ) {
				$this->errors[] = sprintf( __( 'Unable to license %1$s. Reason: %2$s', 'it-l10n-solid-backups' ), $name, $reason );
			}
		}

		if ( ! empty( $warn ) ) {
			foreach ( $warn as $name => $reason ) {
				$this->soft_errors[] = sprintf( __( 'Unable to license %1$s. Reason: %2$s', 'it-l10n-solid-backups' ), $name, $reason );
			}
		}
	}
	...
	private function license_patchstack_site( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership username and password in order to remove patchstack licenses.', 'it-l10n-solid-backups' );
		} else if ( empty( $data['username'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership username in order to remove patchstack licenses.', 'it-l10n-solid-backups' );
		} else if ( empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership password in order to remove patchstack licenses.', 'it-l10n-solid-backups' );
		}

		$keys = Ithemes_Updater_Keys::get();
		if ( empty( $keys['ithemes-security-pro'] ) ) {
			$this->errors[] = __( 'Unable to locate API key for Solid Security Pro.', 'it-l10n-solid-backups' );
		}
		...
		if ( ! empty( $success ) ) {
			$this->messages[] = wp_sprintf( _n( 'Successfully added Patchstack license to %l.', 'Successfully removed Patchstack licenses from %l sites.', count( $success ), 'it-l10n-solid-backups' ), $success );
		}

		if ( ! empty( $fail ) ) {
			foreach ( $fail as $name => $reason ) {
				$this->errors[] = sprintf( __( 'Unable to add patchstack license to %1$s. Reason: %2$s', 'it-l10n-solid-backups' ), $name, $reason );
			}
		}
	}
	...
	private function unlicense_patchstack_sites( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) ) {
			...
			$this->errors[] = __( 'You must select at least one license to remove. Ensure that you select the licenses that you wish to remove in the listing below.', 'it-l10n-solid-backups' );
		}
		...
		if ( ! empty( $success ) ) {
			$this->messages[] = wp_sprintf( _n( 'Successfully removed Patchstack license from %l site.', 'Successfully removed Patchstack licenses from %l sites.', count( $success ), 'it-l10n-solid-backups' ), $success );
		}

		if ( ! empty( $fail ) ) {
			foreach ( $fail as $name => $reason ) {
				$this->errors[] = sprintf( __( 'Unable to remove patchstack license from %1$s. Reason: %2$s', 'it-l10n-solid-backups' ), $name, $reason );
			}
		}
	...
	private function unlicense_packages( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership username and password in order to remove licenses.', 'it-l10n-solid-backups' );
		} else if ( empty( $data['username'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership username in order to remove licenses.', 'it-l10n-solid-backups' );
		} else if ( empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership password in order to remove licenses.', 'it-l10n-solid-backups' );
		} else if ( empty( $data['packages'] ) ) {
			$this->errors[] = __( 'You must select at least one license to remove. Ensure that you select the licenses that you wish to remove in the listing below.', 'it-l10n-solid-backups' );
		}
		...
		if ( empty( $response['packages'] ) ) {
			$this->errors[] = __( 'An unknown server error occurred. Please try to remove licenses from your products again at another time.', 'it-l10n-solid-backups' );
			return;
		}
		...
		foreach ( $response['packages'] as $package => $data ) {
			...
			if ( isset( $data['status'] ) && ( 'inactive' == $data['status'] ) ) {
				...
				$fail[$name] = __( 'Unknown server error.', 'it-l10n-solid-backups' );
			}
		}


		if ( ! empty( $success ) ) {
			$this->messages[] = wp_sprintf( _n( 'Successfully removed license from %l.', 'Successfully removed licenses from %l.', count( $success ), 'it-l10n-solid-backups' ), $success );
		}

		if ( ! empty( $fail ) ) {
			foreach ( $fail as $name => $reason ) {
				$this->errors[] = sprintf( __( 'Unable to remove license from %1$s. Reason: %2$s', 'it-l10n-solid-backups' ), $name, $reason );
			}
		}
	}
	...
	public function list_packages( $action = 'list_packages', $post_data = array() ) {
		...
		if ( ! empty( $_REQUEST['updated_url'] ) ) {
			$this->messages[] = __( 'Successfully updated the Licensed URL.', 'it-l10n-solid-backups' );
		}
?>
	...
	<div class="solidwp-licensing">
		<div class="solidwp-licensing-wrap">
			<div class="solidwp-licensing-wrap-header">
				<h2><?php _e( 'SolidWP Licensing', 'it-l10n-solid-backups' ); ?></h2>
	...
	<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo esc_attr( $this->self_url ); ?>">
		...
		<div id="ithemes-updater-settings">
			...
			<div class="solidwp-table-header">
				<div>
					<h3 class="subtitle"><?php _e( 'Settings', 'it-l10n-solid-backups' ); ?></h3>
				</div>
			</div>
			...
			<table class="form-table">
				<tbody>
					<tr valign="top">
						<th scope="row">
							<?php _e( 'Licensed URL', 'it-l10n-solid-backups' ); ?>
						</th>
						<td>
							<p>
								<code><?php echo esc_html( $GLOBALS['ithemes-updater-settings']->get_licensed_site_url() ); ?></code>
								<a href="<?php echo admin_url( 'options-general.php?page=ithemes-licensing&action=change_licensed_site_url' ); ?>" class="button button-primary"><?php _e( 'Change', 'it-l10n-solid-backups' ); ?></a>
							</p>

							<?php if ( is_multisite() ) : ?>
								<p class="description"><?php _e( 'The Licensed URL should be the primary URL of this WordPress network. If this is not set correctly, some features may not function as expected.', 'it-l10n-solid-backups' ); ?></p>
							<?php else : ?>
								<p class="description"><?php _e( 'The Licensed URL should be the primary URL of this WordPress site. If this is not set correctly, some features may not function as expected.', 'it-l10n-solid-backups' ); ?></p>
							<?php endif; ?>
						</td>
					</tr>
					<tr valign="top">
						<th scope="row">
							<label for="quick_releases"><?php _e( 'Quick Release Updates', 'it-l10n-solid-backups' ); ?></label>
						</th>
						<td>
			...
			<p class="submit">
				<input id="save_settings" class="button button-primary" type="submit" value="<?php _e( 'Save Settings', 'it-l10n-solid-backups' ); ?>" />
...
<?php
	...
	private function show_sunset_banner( $packages ) {
		...
		if ( count( $sunset ) === 1 ) {
			$sunset_text = sprintf(
				__( '%s is being Sunset, but we have alternatives for you!', 'it-l10n-solid-backups' ),
				Ithemes_Updater_Functions::get_package_name( $sunset[0] )
			);
		} else {
			$sunset_text = __( 'iThemes plugins are being Sunset, but we have alternatives for you!', 'it-l10n-solid-backups' );
		}
	...
	private function list_licensed_products( $products, $post_data, $action ) {
		...
		$headings = array(
			__( 'Product', 'it-l10n-solid-backups' ),
			__( 'Member', 'it-l10n-solid-backups' ),
			__( 'Expiration', 'it-l10n-solid-backups' ),
			__( 'Remaining Licenses', 'it-l10n-solid-backups' ),
		);
...
?>
	<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo esc_attr( $this->self_url ); ?>" autocomplete="off">
		...
		<div class="ithemes-updater-products" id="ithemes-updater-licensed">
			<div class="solidwp-table-header">
				<div>
					<h3 class="subtitle"><?php _e( 'Licensed Products', 'it-l10n-solid-backups' ); ?></h3>
				</div>
			</div>
			...
			<table class="ithemes-updater-listing widefat">
				<thead>
					<tr>
						...
						<th scope="col">
							<label for="cb-select-all-1"><?php _e( 'Product', 'it-l10n-solid-backups' ); ?></label>
						</th>
						<th scope="col" class="mobile-hidden"><?php _e( 'Member', 'it-l10n-solid-backups' ); ?></th>
						<th scope="col"><?php _e( 'Product Status', 'it-l10n-solid-backups' ); ?></th>
						<th scope="col" class="mobile-hidden"><?php _e( 'Expiration', 'it-l10n-solid-backups' ); ?></th>
						<th scope="col" class="mobile-hidden"><?php _e( 'Remaining Licenses', 'it-l10n-solid-backups' ); ?></th>
					</tr>
				</thead>
				<tbody>
					<?php $count = 0; ?>
					<?php foreach ( $products as $name => $data ) : ?>
						<?php
							if ( -1 == $data['total'] ) {
								$remaining = __( 'unlimited', 'it-l10n-solid-backups' );
							} else {
								$remaining = $data['total'] - $data['used'];
							}

//							if ( 0 == $remaining )
//								$remaining .= ' <a class="button-secondary upgrade">' . __( 'Upgrade', 'it-l10n-solid-backups' ) . '</a>';
				...
				<tfoot>
					<tr>
						<td colspan="6">
							...
							<input class="button-primary" type="submit" name="submit" value="<?php _e( 'Remove Licenses', 'it-l10n-solid-backups' ); ?>" />
...
?>
	...
	<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo esc_attr( $this->self_url ); ?>" autocomplete="off">
		...
		<div class="ithemes-updater-products border-bottom" id="ithemes-updater-licensed">
			<div class="solidwp-table-header">
				<div>
					<h3 class="subtitle"><?php _e( 'Patchstack Enabled', 'it-l10n-solid-backups' ); ?></h3>
				</div>
			</div>
			...
			<table class="ithemes-updater-listing widefat">
				<tbody>
					<tr>
						<td colspan="2">
							<p><?php printf( __( 'You have %d Remaining Site Licenses', 'it-l10n-solid-backups' ), $patchstack_available ); ?></p>
							<p><strong><?php _e( 'This site is licensed with Patchstack.', 'it-l10n-solid-backups' ); ?></strong></p>
							<p><a href="https://my.solidwp.com/panel/patchstack.php"><?php _e( 'Click here to manage your Patchstack licenses.', 'it-l10n-solid-backups' ); ?></a></p>
						</td>
					</tr>
					<?php $count = 0; ?>
				...
				<tfoot>
					<tr>
						<td colspan="2">
							<input type="text" name="it-updater-username" placeholder="SolidWP Username" value="<?php echo esc_attr( $post_data['username'] ); ?>" autocomplete="off" />
							<input type="password" name="it-updater-password" placeholder="Password" value="<?php echo esc_attr( $post_data['password'] ); ?>" />
							<input class="button-primary" type="submit" name="submit" value="<?php _e( 'Remove Patchstack License', 'it-l10n-solid-backups' ); ?>" />
	...
	<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo esc_attr( $this->self_url ); ?>" autocomplete="off">
		<div class="ithemes-updater-products" id="ithemes-updater-licensed">
			<div class="solidwp-table-header border-bottom">
				<div>
					<h3 class="subtitle"><?php _e( 'Enable Patchstack', 'it-l10n-solid-backups' ); ?></h3>
				</div>
			</div>
			<table class="ithemes-updater-listing widefat">
				<tbody>
					<tr><td>
						<p class="unlicensed-patchstack-paragraph"><?php printf( __( 'You have <strong>%d Remaining Site Licenses</strong>', 'it-l10n-solid-backups' ), $patchstack_available ); ?></p>
						<p><?php _e( '<strong>This site is not licensed with Patchstack.</strong>', 'it-l10n-solid-backups' ); ?></p>
						<p><?php printf( __( 'License your site below using your SolidWP username and password or <a href="%s">click here to manage your Patchstack licenses.</a>', 'it-l10n-solid-backups' ), 'https://my.solidwp.com/panel/patchstack.php' ); ?></p>
					</td></tr>
				</tbody>
				<tfoot>
					<tr>
						<td>
							...
							<input type="text" name="it-updater-username" placeholder="SolidWP Username" value="<?php echo esc_attr( $post_data['username'] ); ?>" autocomplete="off" />
							<input type="password" name="it-updater-password" placeholder="Password" value="<?php echo esc_attr( $post_data['password'] ); ?>" />
							<input class="button-secondary" type="submit" name="submit" value="<?php _e( 'License This Site', 'it-l10n-solid-backups' ); ?>" />
...
?>
	<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo esc_attr( $this->self_url ); ?>" autocomplete="off">
		...
		<div class="ithemes-updater-products" id="ithemes-updater-unlicensed">
			<div class="solidwp-table-header">
				<div>
					<h3><?php _e( 'Unlicensed Products', 'it-l10n-solid-backups' ); ?></h3>
					<p><?php _e( 'The following products have not been licensed. Licensing a product gives you access to automatic updates from within WordPress.', 'it-l10n-solid-backups' ); ?></p>
					<p><?php _e( 'To license products, select the products you wish to license, enter your SolidWP membership username and password, and press the License Products button.', 'it-l10n-solid-backups' ); ?></p>
					<p><?php printf( __( 'Need help? <a href="%s">Click here for a quick video tutorial</a>.', 'it-l10n-solid-backups' ), 'https://go.solidwp.com/solidwp-updater' ); ?></p>
				</div>
			</div>
			...
			<table class="ithemes-updater-listing widefat">
				<thead>
					<tr>
						...
						<th scope="col">
							<label for="cb-select-all-2"><?php _e( 'Product', 'it-l10n-solid-backups' ); ?></label>
						</th>
					</tr>
				</thead>
				<tbody>
				...
				<tfoot>
					<tr>
						<td colspan="2">
							<input type="text" name="it-updater-username" placeholder="SolidWP Username" value="<?php echo esc_attr( $post_data['username'] ); ?>" autocomplete="off" />
							<input type="password" name="it-updater-password" placeholder="Password" value="<?php echo esc_attr( $post_data['password'] ); ?>" />
							<input class="button-primary" type="submit" name="submit" value="<?php _e( 'License Products', 'it-l10n-solid-backups' ); ?>" />
...
?>
	<div class="ithemes-updater-products" id="ithemes-updater-unrecognized">
		<h3 class="subtitle"><?php _e( 'Unrecognized Products', 'it-l10n-solid-backups' ); ?></h3>

		<p><?php _e( 'The following products were not recognized by the licensing system. This can be due to a bug in the product code, a temporary server issue, or because the product is no longer supported.', 'it-l10n-solid-backups' ); ?></p>
		<p><?php printf( __( 'Please check this page again at a later time to see if the problem resolves itself. If the product remains, please contact <a href="%s">SolidWP support</a> and provide them with the details given below.', 'it-l10n-solid-backups' ), 'https://solidwp.com/support/' ); ?></p>

		<table class="ithemes-updater-listing widefat">
			<thead>
				<tr>
					<th scope="col"><?php _e( 'Product', 'it-l10n-solid-backups' ); ?></th>
					<th scope="col"><?php _e( 'Type', 'it-l10n-solid-backups' ); ?></th>
					<th scope="col"><?php _e( 'Package', 'it-l10n-solid-backups' ); ?></th>
					<th scope="col"><?php _e( 'Version', 'it-l10n-solid-backups' ); ?></th>
					<th scope="col"><?php _e( 'Server Response', 'it-l10n-solid-backups' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php $count = 0; ?>
				<?php foreach ( $products as $name => $data ) : ?>
					<?php
						if ( ( isset( $data['status'] ) && 'error' == $data['status'] ) && ( ! empty( $data['error']['message'] ) ) ) {
							...
							$response = __( 'Unknown Error', 'it-l10n-solid-backups' );
						}
...
?>
	<div class="wrap" id="ithemes-updater-site-url-confirmation">
		...
		<div class="solidwp-licensing">
			<div class="solidwp-licensing-wrap">
				<div class="solidwp-licensing-wrap-header">
					<h2><?php _e( 'Licensing', 'it-l10n-solid-backups' ); ?></h2>
					<p><?php _e( "Please confirm this site's licensed URL.", 'it-l10n-solid-backups' ); ?></p>
				</div>
				...
				<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo esc_attr( $this->self_url ); ?>">
					...
					<div id="ithemes-updater-settings">
						<div id="ithemes-updater-table-wrapper">
							<table class="form-table">
								<tbody>
									<tr valign="top">
										<th scope="row">
											<label for="site_url"><?php _e( 'Licensed URL', 'it-l10n-solid-backups' ); ?></label>

											<?php if ( is_multisite() ) : ?>
												<p class="description"><?php _e( 'The Licensed URL should be the primary URL of this WordPress network.', 'it-l10n-solid-backups' ); ?></p>
												<p class="description ithemes-updater-description-warning"><?php _e( 'If not set correctly, some features may not function as expected.', 'it-l10n-solid-backups' ); ?></p>
											<?php else : ?>
												<p class="description"><?php _e( 'The Licensed URL should be the primary URL of this WordPress site.', 'it-l10n-solid-backups' ); ?></p>
												<p class="description ithemes-updater-description-warning"><?php _e( 'If not set correctly, some features may not function as expected.', 'it-l10n-solid-backups' ); ?></p>
											<?php endif; ?>
										</th>
									</tr>
						...
						<p class="submit">
							<input id="save_licensed_site_url" class="button button-primary" type="submit" value="<?php _e( 'Save', 'it-l10n-solid-backups' ); ?>" />
...
<?php
	...
	private function save_licensed_site_url( $data ) {
		...
		if ( empty( $data['site_url'] ) ) {
			$this->errors[] = __( 'The licensed URL cannot be blank.', 'it-l10n-solid-backups' );
		} else if ( false === filter_var( $data['site_url'], FILTER_VALIDATE_URL ) ) {
			$this->errors[] = __( 'The licensed URL must be a valid URL.', 'it-l10n-solid-backups' );
		}
		...
		$this->messages[] = __( 'Successfully set the Licensed URL.', 'it-l10n-solid-backups' );
...
?>
	<div class="wrap" id="ithemes-updater-relicense">
		...
		<div class="solidwp-licensing-wrap">
			...
			<p><?php printf( __( 'The licenses on this site are for <code>%s</code>.', 'it-l10n-solid-backups' ), esc_html( $site_url_from_server ) ); ?></p>
			...
			<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo esc_attr( $this->self_url ); ?>">
				...
				<div id="ithemes-updater-settings">
					<div id="ithemes-updater-table-wrapper">
						<h3><?php _e( 'License Option', 'it-l10n-solid-backups' ); ?></h3>
						<p>
							<label>
								<input type="radio" name="relicense_option" value="relicense" <?php if ( 'relicense' === $data['relicense_option'] ) {echo 'checked="checked"';} ?> />
								<?php _e( 'Create new licenses for this site.', 'it-l10n-solid-backups' ); ?>
							</label>
						</p>
						<p class="description ithemes-updater-description-warning"><?php _e( 'Use this option if this site was cloned from another site and needs to have its own licenses.', 'it-l10n-solid-backups' ); ?></p>
						<p>
							<label>
								<input type="radio" name="relicense_option" value="update" <?php if ( 'update' === $data['relicense_option'] ) {echo 'checked="checked"';} ?> />
								<?php printf( __( 'Change the existing licenses to be for <code>%s</code>.', 'it-l10n-solid-backups' ), esc_html( $data['site_url'] ) ); ?>
							</label>
						</p>
						<p class="description ithemes-updater-description-notice">
							<span class="dashicons dashicons-warning"></span>
							<?php printf( __( 'Note: If the <code>%s</code> site still exists and is different from this site, you will have to create new licenses on that site.', 'it-l10n-solid-backups' ), esc_html( $data['site_url'] ) ); ?>
						</p>
						<p class="description ithemes-updater-description-warning secondary-description"><?php printf( __( 'Use this option if this site\'s primary URL has changed from <code>%1$s</code> to <code>%2$s</code>.', 'it-l10n-solid-backups' ), esc_html( $site_url_from_server ), esc_html( $data['site_url'] ) ); ?></p>
						<div class="input-container">
							<label for="it-updater-username"><?php _e( 'SolidWP Username', 'it-l10n-solid-backups' ); ?></label>
							<input id="it-updater-username" type="text" name="it-updater-username" value="<?php echo esc_attr( $data['username'] ); ?>" />
							<label for="it-updater-password"><?php _e( 'SolidWP Password', 'it-l10n-solid-backups' ); ?></label>
							<input id="it-updater-password" type="password" name="it-updater-password" value="<?php echo esc_attr( $data['password'] ); ?>" />
						</div>
					</div>
				</div>
				<p class="submit">
					<input id="relicense" class="button button-primary" type="submit" value="<?php _e( 'Save', 'it-l10n-solid-backups' ); ?>" />
...
<?php
	...
	private function relicense( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership username and password in order to change the licensed URL.', 'it-l10n-solid-backups' );
		} else if ( empty( $data['username'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership username in order to change the licensed URL.', 'it-l10n-solid-backups' );
		} else if ( empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership password in order to change the licensed URL.', 'it-l10n-solid-backups' );
		} else if ( empty( $data['site_url'] ) ) {
			$this->errors[] = __( 'The licensed URL cannot be blank.', 'it-l10n-solid-backups' );
		} else if ( false === filter_var( $data['site_url'], FILTER_VALIDATE_URL ) ) {
			$this->errors[] = __( 'The licensed URL must be a valid URL.', 'it-l10n-solid-backups' );
		} else if ( empty( $data['relicense_option'] ) || ! in_array( $data['relicense_option'], array( 'relicense', 'update' ) ) ) {
			$this->errors[] = __( 'You must pick one of the License Option options.', 'it-l10n-solid-backups' );
		}
	...
	private function get_expiration_string( $expiration_timestamp ) {
		...
		if ( $time_left > ( 86400 * 30 ) ) {
			...
			if ( $time_left > 86400 ) {
				$expiration = sprintf( _n( '%d day', '%d days', intval( $time_left / 86400 ), 'it-l10n-solid-backups' ), intval( $time_left / 86400 ) );
			} else if ( $time_left > 3600 ) {
				$expiration = sprintf( _n( '%d hour', '%d hours', intval( $time_left / 3600 ), 'it-l10n-solid-backups' ), intval( $time_left / 3600 ) );
			} else if ( $time_left > 60 ) {
				$expiration = sprintf( _n( '%d minute', '%d minutes', intval( $time_left / 60 ), 'it-l10n-solid-backups' ), intval( $time_left / 60 ) );
			} else {
				$expiration = sprintf( _n( '%d second', '%d seconds', $time_left, 'it-l10n-solid-backups' ), intval( $time_left / 60 ) );
			}

			if ( $expired ) {
				$expiration = sprintf( __( '%s ago', 'it-l10n-solid-backups' ), $expiration );
			}
		}
