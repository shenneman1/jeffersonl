...
class Ithemes_Updater_API {
	...
	public static function get_package_changelog( $package, $cur_version = false ) {
		...
		if ( ! isset( $response['body'] ) ) {
			return new WP_Error( 'ithemes-updater-changelog-bad-wp-remote-get-response', __( 'Unrecognized response from <code>wp_remote_get</code>.', 'it-l10n-solid-backups' ) );
		}
		...
		if ( '{' === substr( $body, 0, 1 ) ) {
			...
			if ( is_array( $error ) && isset( $error['error'] ) && is_array( $error['error'] ) && isset( $error['error']['type'] ) && isset( $error['error']['message'] ) ) {
				...
				return new WP_Error( 'ithemes-updater-changelog-bad-json-decode-result', __( 'Unrecognized response from SolidWP API server.', 'it-l10n-solid-backups' ) );
			}
		}
		...
		if ( false === $response ) {
			switch ( $action ) {
				...
				default:
					...
					$response = new WP_Error( 'ithemes-updater-unrecognized-action', sprintf( __( 'Unrecognized action passed to %s', 'it-l10n-solid-backups' ), 'Ithemes_Updater_API::get_response()' ) );
					break;
			}
	...
	public static function get_error_explanation( $error, $package = '' ) {
		...
		switch ( $code ) {
			case 'ITXAPI_Updater_Bad_Login':
				$message = __( 'Incorrect password. Please make sure that you are supplying your SolidWP membership username and password details.', 'it-l10n-solid-backups' );
				break;
			case 'ITXAPI_Updater_Username_Unknown':
			case 'ITXAPI_Updater_Username_Invalid':
				$message = __( 'Invalid username. Please make sure that you are supplying your SolidWP membership username and password details.', 'it-l10n-solid-backups' );
				break;
			case 'ITXAPI_Product_Package_Unknown':
				/* translators: 1: Product name, 2: Package name. */
				$message = sprintf( __( 'The licensing server reports that the %1$s (%2$s) product is unknown. Please contact support for assistance.', 'it-l10n-solid-backups' ), $package_name, $package );
				break;
			case 'ITXAPI_Updater_Too_Many_Sites':
				/* translators: 1: Product name. */
				$message = sprintf( __( '%1$s could not be licensed since the membership account is out of available licenses for this product. You can unlicense the product on other sites or upgrade your membership to one with a higher number of licenses in order to increase the amount of available licenses.', 'it-l10n-solid-backups' ), $package_name );
				break;
			case 'ITXAPI_License_Key_Generate_Failed':
				/* translators: 1: Product name. */
				$message = sprintf( __( '%1$s could not be licensed due to an internal error. Please try to license %2$s again at a later time. If this problem continues, please contact SolidWP support.', 'it-l10n-solid-backups' ), $package_name );
				break;
		}
		...
		if ( empty( $message ) ) {
			if ( ! empty( $package ) ) {
				...
				$message = sprintf( __( 'An unknown error relating to the %1$s product occurred. Please contact SolidWP support. Error details: %2$s', 'it-l10n-solid-backups' ), $package_name, $error->get_error_message() . " ($code)" );
			} else {
				/* translators: 1: error code. */
				$message = sprintf( __( 'An unknown error occurred. Please contact SolidWP support. Error details: %s', 'it-l10n-solid-backups' ), $error->get_error_message() . " ($code)" );
			}
		}
