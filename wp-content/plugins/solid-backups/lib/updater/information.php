...
class Ithemes_Updater_Information {
	...
	public static function get_plugin_information( $path ) {
		...
		if ( is_wp_error( $changelog ) ) {
			...
			$changelog = sprintf( __( '<p>Unable to get changelog data at this time.</p><p>%1$s (%2$s)</p>', 'it-l10n-solid-backups' ), esc_html( $changelog->get_error_message() ), esc_html( $changelog->get_error_code() ) );
		}
